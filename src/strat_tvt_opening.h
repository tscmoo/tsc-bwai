		...
		while (true) {
			...
			for (unit*e : enemy_units) {
				if (e->type == unit_types::barracks || e->type == unit_types::scv || e->type == unit_types::marine) {
					if (unit_pathing_distance(unit_types::scv, e->pos, combat::my_closest_base) < unit_pathing_distance(unit_types::scv, e->pos, combat::op_closest_base)) {
			...
			bool defend_proxy = (proxy_barracks_count || proxy_marine_count || proxy_scv_count >= 4) && !defence_ok;
