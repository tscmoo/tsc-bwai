...
struct strat_tank {
	...
	void run() {
		...
		while (true) {
			...
			combat::no_aggressive_groups = my_tank_count < 12;
			...
			int desired_goliath_count = enemy_carrier_count * 3;
			...
			auto can_expand = [&]() {
				if (buildpred::get_my_current_state().bases.size() <= 2 && my_tank_count < 12) return false;
