//
// This file implements the multitasking interface.
//
	...
	namespace multitasking {
		struct multitasking_t::impl_t {
			...
			bool schedule() {
				...
				for (task_id id : running_tasks) {
					...
					if (!t.raise_signal) {
						...
						if (t.sleep_until && bot.current_frame < t.sleep_until) continue;
			...
			void resume() {
				...
				else if (frame_time_carry < -bot.multitasking.desired_frame_time) frame_time_carry = -bot.multitasking.desired_frame_time;
			}
			...
			task_id spawn(double prio, std::function<void()> f, const char*name) {
				...
				*std::get<1>(v) = [this, f = std::move(f), t, v]() {
					try {
						current_task = t;
						check_signals();
						f();
					} catch (const sig_term&) {
						bot.log("%s: terminated\n", current_task->name);
					} catch (const std::exception&e) {
						bot.log("%s: caught exception : %s\n", current_task->name, e.what());
						std::terminate();
					} catch (const char*str) {
						bot.log("%s: caught exception : %s\n", current_task->name, str);
						std::terminate();
					} catch (...) {
						bot.log("%s: caught unknown exception\n", current_task->name);
						std::terminate();
					}
			...
			void sleep(int frames) {
				current_task->sleep_until = bot.current_frame + frames;
				schedule();
			}
