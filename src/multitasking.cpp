...
namespace multitasking {
	...
	multitasking::task_id spawn(double prio, std::function<void()> f, const char*name) {
		return detail::spawn(prio, std::move(f), name);
	}
	...
	void stop() {
		...
		detail::resume();
		if (detail::running_tasks.any()) xcept("internal error: tasks still running after terminate");
	}
